...
public class PersistenceAndBeanConvertWithAction {
	...
	public static ZeusActionWithBLOBs convert(Tuple<JobDescriptor, JobStatus> job) {
		...
		ZeusActionWithBLOBs persist = convert(job.getX());
		ZeusActionWithBLOBs p2 = convert(job.getY());
...
/*		jd.setScript(persist.getScript());*/
	...
	public static ZeusActionWithBLOBs convert(JobDescriptor jd) {
		...
		persist.setJobId(Long.valueOf(jd.getJobId()));
...
/*		persist.setScript(jd.getScript());*/
	...
	public static DebugHistory convert(ZeusDebugHistoryWithBLOBs persistence) {
