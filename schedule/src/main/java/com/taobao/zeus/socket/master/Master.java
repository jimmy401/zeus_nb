...
*//*									actionPer.setScript(jobDetail.getScript());*//*
                            ...
                            if (relyedMostDepsActions != null && relyedMostDepsActions.size() > 0
                                ...
                                try {
                                    ...
                                    for (String depJobId : dependStrs) {
                                        ...
                                        for (ZeusActionWithBLOBs item:actionItems){
                                            if (actionId<item.getId()){
                                                actionId=item.getId();
                                            }
                                        }
                                        if (actionMostDeps.equalsIgnoreCase(depJobId)){
                                            actionMostId=actionId;
                                        }
                                        if (actionDependencies.trim().length() > 0) {
                                            actionDependencies += ",";
                                        }
                                        actionDependencies += String.valueOf((actionId / 1000000) * 1000000 + Long.parseLong(depJobId));
                                    }
    ...
    private void saveZeusAction(Map<Long, ZeusActionWithBLOBs> actionDetails, ZeusJobWithBLOBs jobDetail, String jobDependencies, String actionDependencies, Long actionId) {
        ZeusActionWithBLOBs actionPer = new ZeusActionWithBLOBs();
        actionPer.setId(actionId);//update action id
        actionPer.setJobId(jobDetail.getId());
        actionPer.setAuto(jobDetail.getAuto());
        actionPer.setConfigs(jobDetail.getConfigs());
        actionPer.setCronExpression(jobDetail.getCronExpression());//update action cron expression
