...
public class WorkerBeExecute {
    ...
    public Future<Response> manual(final WorkerContext context,final Request req) {
        ...
        final ZeusActionHistory history = context.getJobHistoryManager()
        ...
        Future<Response> f = context.getThreadPool().submit(
                new Callable<Response>() {
                    public Response call() throws Exception {
                        ...
                        File direcotry = new File(Environment.getDownloadPath() + File.separator + date + File.separator + "manual-" + history.getId());
                        if (!direcotry.exists()) {
                            direcotry.mkdirs();
                        }
                        JobBean jb = context.getGroupManager()
                                .getUpstreamJobBean(history.getActionId());

                        final Job job = JobUtils.createJob(new JobContext(JobContext.MANUAL_RUN),
                                jb, history, direcotry.getAbsolutePath(), context.getApplicationContext());
                        ...
                        try {
                            ...
                            zeusActionHistory.setEndTime(new Date());
                            if (exitCode == 0) {
                                zeusActionHistory.setStatus(com.taobao.zeus.model.JobStatus.Status.SUCCESS);
                            } else {
                                zeusActionHistory.setStatus(com.taobao.zeus.model.JobStatus.Status.FAILED);
                            }
                            context.getJobHistoryManager().updateJobHistory(zeusActionHistory);
                        ...
                        Response resp = Response.newBuilder().setRid(req.getRid()).setOperate(Operate.Manual).setStatus(status).setErrorText(errorText).build();
                        SocketLog.info("send manual response,manual complete,rid="
                                + req.getRid()
                                + ",historyId="
                                + historyId);
                        return resp;
                ...
                new Callable<Response>() {
                    public Response call() throws Exception {
                        ...
                        final Job job = JobUtils.createDebugJob(
                                new JobContext(JobContext.DEBUG_RUN), history, direcotry.getAbsolutePath(), context.getApplicationContext());
                        ...
                        try {
                            ...
                            DebugHistory debugHistory = context.getDebugHistoryManager().findDebugHistory(history.getId());
                            debugHistory.setEndTime(new Date());
                            if (exitCode == 0) {
                                debugHistory.setStatus(com.taobao.zeus.model.JobStatus.Status.SUCCESS);
                            } else {
                                debugHistory.setStatus(com.taobao.zeus.model.JobStatus.Status.FAILED);
                            }
                            context.getDebugHistoryManager().updateDebugHistory(debugHistory);
                            history.getLog().appendZeus("exitCode=" + exitCode);
                            try {
                                context.getDebugHistoryManager().updateDebugHistoryLog(history.getId(), history.getLog().getContent());
                            } catch (Exception ex) {
                        ...
                        Response resp = Response.newBuilder().setRid(
                                req.getRid()).setOperate(Operate.Debug).setStatus(status).setErrorText(errorText).build();
                        SocketLog.info("send debug response,debug complete,rid="
    ...
    private Future<Response> schedule(final WorkerContext context, final Request req) {
        ...
        if (context.getRunnings().containsKey(jobId)) {
            SocketLog
                    .info("send execute response,job is running and can't runAction again,rid="
                            + req.getRid() + ",jobId=" + em.getJobId());
        ...
        final JobStatus js = context.getGroupManager().getActionStatus(jobId);
        final ZeusActionHistory history = context.getJobHistoryManager().findJobHistory(js.getHistoryId());
        Future<Response> f = context.getThreadPool().submit(
                new Callable<Response>() {
                    public Response call() throws Exception {
                        history.setExecuteHost(WorkerContext.host);
                        ...
                        JobBean jb = context.getGroupManager().getUpstreamJobBean(history.getActionId());
                        String date = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
                        File direcotry = new File(Environment.getDownloadPath() + File.separator + date + File.separator + history.getId());
                        if (!direcotry.exists()) {
                            direcotry.mkdirs();
                        }
                        ...
                        Response resp = Response.newBuilder().setRid(req.getRid()).setOperate(Operate.Schedule).setStatus(status).setErrorText(errorText).build();
                        SocketLog.info("send execute response,execute complete,rid="
                                + req.getRid()
                                + ",jobId="
                                + history.getActionId());
                        return resp;
