...
public class WorkerWebExecute {
	...
	public Future<WebResponse> send(final WorkerContext context,ExecuteKind kind,String id){
		...
		final WebRequest req=WebRequest.newBuilder().setRid(AtomicIncrease.getAndIncrement()).
				setOperate(WebOperate.ExecuteJob)
			.setEk(kind).setId(id).build();
		SocketMessage sm=SocketMessage.newBuilder().
				setKind(Kind.WEB_REUQEST).setBody(req.toByteString()).build();
